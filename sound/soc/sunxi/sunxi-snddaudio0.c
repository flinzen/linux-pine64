					...
					struct snd_pcm_hw_params *params)
{
	...
	pr_info("[daudio0], codec_dai_name %s\n", codec_dai->name);
	pr_info("[daudio0], codec_dai_probed %d\n", codec_dai->probed);
	if (ret < 0) {
		pr_warn("[daudio0],the codec_dai set set_sysclk failed%d\n", ret);
	}

	ret = snd_soc_dai_set_fmt(codec_dai, SND_SOC_DAIFMT_I2S |
						SND_SOC_DAIFMT_NB_NF | SND_SOC_DAIFMT_CBS_CFS);
	if (ret < 0) {
		pr_warn("[daudio0],the codec_dai set set_fmt failed%d\n", ret);
	}
	/*
	...
	*/
	...
	if (ret < 0) {
		pr_warn("[daudio0],the codec_dai set set_clkdiv failed%d\n", ret);
	}
...
static int  sunxi_snddaudio0_dev_probe(struct platform_device *pdev)
{
	...
	pr_info("[daudio0]codec_of_node: %s\n", sunxi_snddaudio_dai_link.codec_of_node->name);
