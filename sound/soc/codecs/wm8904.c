...
SOC_SINGLE("Mic Bias Switch", WM8904_MIC_BIAS_CONTROL_0, 0, 1, 0),
				 ...
				 enum snd_soc_bias_level level)
{
	struct wm8904_priv *wm8904 = snd_soc_codec_get_drvdata(codec);
	int ret;
	printk(KERN_ERR "wm8904_set_bias_level %d\n", (int)level);
	switch (level) {
	case SND_SOC_BIAS_ON:
		break;

	case SND_SOC_BIAS_PREPARE:
		/* snd_soc_update_bits(codec, WM8904_MIC_BIAS_CONTROL_0, WM8904_MICBIAS_ENA_MASK, WM8904_MICBIAS_ENA); */
			    ...
			    const struct i2c_device_id *id)
{
	...
	regmap_update_bits(wm8904->regmap, WM8904_MIC_BIAS_CONTROL_0, 0xffff, 1);
	if (wm8904->pdata) {
