							...
							struct pn544_i2c_platform_data *pdata)
{
	...
	ven_val = of_get_named_gpio_flags(node, "enable-gpios", 0, &flags);
	if (ven_val >= 0) {
		pr_err("%d gpio", ven_val);
		pdata->ven_gpio = ven_val;
	}
	else {
	...
	val = -1; // of_get_named_gpio_flags(node, "firmware-gpios", 0, &flags);
	if (val >= 0) {
		pdata->firm_gpio = val;
	}
	else {
	...
	irq_val = 71; // of_get_named_gpio_flags(node, "interrupt-gpios", 0, &flags);
	if (irq_val >= 0) {
		pdata->irq_gpio = irq_val;
	}
	else {
	...
	val = -1; // of_get_named_gpio_flags(node, "nxp,pn54x-clkreq", 0, &flags);
	if (val >= 0) {
